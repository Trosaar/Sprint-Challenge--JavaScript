...
const contactorInfo = [];
for (let i = 0; i < graduates.length; i++) {
  contactorInfo.push(`${graduates[i].first_name} ${graduates[i].email}`)};
...
const uni = [];
for (let i = 0; i < graduates.length; i++) {
  if (graduates[i].university.toLowerCase().includes("uni") === true) {
...
*/
const animalNames = [];
zooAnimals.forEach(function(anim){
  animalNames.push({name: anim.animal_name, scientific: anim.scientific_name})
});
...
const populationTotal = zooAnimals.reduce(function(accumulator, zooAnimalAge) {accumulator + zooAnimalAge.population}, 0);
